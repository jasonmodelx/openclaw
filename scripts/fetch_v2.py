...
def send_error_notification(channel: str, error_msg: str, details: str = "") -> None:
    """发送错误通知邮件"""
    email_cfg = CONFIG.get("email", {})
    if not email_cfg.get("enabled"):
        return
    ...
    try:
        msg = MIMEMultipart("alternative")
        msg["Subject"] = f"⚠️ 播客任务出错 - {channel}"
        msg["From"] = email_cfg.get("smtp_user")
        msg["To"] = email_cfg.get("to")
...
def send_daily_summary(podcasts_status: list) -> None:
    """发送执行结果通知（列出成功/失败/无新内容的频道）"""
    email_cfg = CONFIG.get("email", {})
    if not email_cfg.get("enabled"):
        return

    # 分类统计
    success_channels = []  # 成功获取新内容的
    no_new_channels = []  # 没有新内容的
    failed_channels = []  # 执行失败的
    
    for p in podcasts_status:
        status = p.get("status", "")
        if "已处理，跳过" in status or "无新" in status:
            no_new_channels.append(p["name"])
        elif p.get("success") and "处理成功" in status:
            success_channels.append(p["name"])
        else:
            failed_channels.append(p["name"])

    # 只有在执行完成后才发通知
    try:
        msg = MIMEMultipart("alternative")
        msg["Subject"] = f"📻 播客任务执行完成 - {TODAY}"
        msg["From"] = email_cfg.get("smtp_user")
        msg["To"] = email_cfg.get("to")

        content_lines = [f"# 📻 播客任务执行完成\n", f"**时间**: {datetime.now(CST).strftime('%Y-%m-%d %H:%M:%S')}\n"]

        # 成功获取新内容
        if success_channels:
            content_lines.append(f"\n## ✅ 获得新内容 ({len(success_channels)})\n")
            for name in success_channels:
                content_lines.append(f"- {name}")

        # 没有新内容
        if no_new_channels:
            content_lines.append(f"\n## ⏭️ 无新内容 ({len(no_new_channels)})\n")
            for name in no_new_channels:
                content_lines.append(f"- {name}")

        # 执行失败
        if failed_channels:
            content_lines.append(f"\n## ❌ 执行失败 ({len(failed_channels)})\n")
            for name in failed_channels:
                content_lines.append(f"- {name}")

        content_lines.append(f"\n---\n由小白自动发送 🐰")

        msg.attach(MIMEText("\n".join(content_lines), "plain", "utf-8"))
...
async def process_podcast(client: NotebookLMClient, podcast: dict) -> str:
    """处理单个播客频道的完整流程（RSS 模式）
    返回状态: "no_new" | "success" | "error"
    """
...
async def process_podcast_with_retry(client: NotebookLMClient, podcast: dict) -> dict:
    """带重试机制的播客处理，返回状态字典"""
    name = podcast.get("name", "Unknown")
    result = {"name": name, "success": False, "status": "未知错误", "error": None}
    
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            status = await process_podcast(client, podcast)
            ...
            if status == "success":
                ...
                result["success"] = True  # 不是错误，只是没有新内容
                result["status"] = "无新内容"
                return result
            else:  # error
                raise Exception("处理返回错误状态")
                
        except Exception as e:
            error_msg = str(e)
            result["error"] = error_msg
            if attempt < MAX_RETRIES:
                print(f"  ⚠️ 第 {attempt} 次失败，{RETRY_DELAY}秒后重试...")
                await asyncio.sleep(RETRY_DELAY)
            else:
                result["status"] = f"执行失败 (已重试{MAX_RETRIES}次)"
    ...
    async with await NotebookLMClient.from_storage() as client:
        ...
        # 批量 RSS 模式
            ...
            if not podcasts:
                print("⚠️ 没有启用的播客频道")
                # 发送空状态简报
                send_daily_summary([{"name": "无启用的频道", "success": False, "status": "无启用的频道"}])
                return
            
            # 并发处理所有启用的播客频道（带重试）
            results = await asyncio.gather(
                *[process_podcast_with_retry(client, p) for p in podcasts],
            ...
            for i, r in enumerate(results):
                if isinstance(r, Exception):
                    podcast_status.append({
                        "name": podcasts[i].get("name", "Unknown"),
                        "success": False,
                        "status": f"异常: {str(r)}"
                    })
                else:
                    podcast_status.append(r)
            
            # 发送每日简报（无论有没有新内容都发）
            send_daily_summary(podcast_status)
